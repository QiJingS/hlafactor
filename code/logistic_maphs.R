log_marginal_prior_horseshoe <- function(beta_j, tau) {
  integrand <- function(s) {
    val <- (1 / s) * exp(- (beta_j^2) / (2 * tau^2 * s * (1 + s)))
    return(val)
  }

  integral <- integrate(integrand, lower = 1e-6, upper = 1000, rel.tol = 1e-8)$value
  const <- -log(pi) - 0.5 * log(2 * pi) - log(tau)

  log_density <- const + log(integral)
  return(log_density)
}


log_marginal_prior_reg_horseshoe <- function(beta_j, tau, c = 1) {
  integrand <- function(s) {
    lambda2_tilde <- (c^2 * s) / (c^2 + tau^2 * s)
    dens_norm <- 1 / sqrt(2 * pi * tau^2 * lambda2_tilde) * exp(-beta_j^2 / (2 * tau^2 * lambda2_tilde))
    dens_lambda <- 1 / (pi * (1 + s) * sqrt(s))
    dens_norm * dens_lambda
  }
...
logitic_our_log_posterior_reg_horseshoe <- function(Y, X, parameters, tau = 0.1, alpha = 0.5, c = 1) {
  beta0 <- parameters[1]
  beta <- parameters[-1]
...
logitic_our_log_prior_reg_horseshoe <- function(Y, X, parameters, tau = 0.1, alpha = 0.5, c = 1) {
  beta0 <- parameters[1]
  beta <- parameters[-1]
...
logitic_our_log_likelihood_reg_horseshoe <- function(Y, X, parameters, tau = 0.1, alpha = 0.5, c = 1) {
  beta0 <- parameters[1]
  beta <- parameters[-1]
  logodds <- beta0 + X %*% beta
  p <- 1 / (1 + exp(-logodds))
...
make_dom_rec_matrix <- function(X) 
{
    Xmat <- as.matrix(X)
    p <- ncol(Xmat)
    n <- nrow(Xmat)
    snp_names <- colnames(Xmat)
    if (is.null(snp_names)) 
        snp_names <- paste0("snp", seq_len(p))
    add <- apply(Xmat, 2, function(col) suppressWarnings(as.numeric(as.character(col))))
    dom <- ifelse(is.na(add), NA_real_, ifelse(add > 0, 1, 0))
    rec <- ifelse(is.na(add), NA_real_, ifelse(add == 2, 1, 0))
    out <- matrix(NA_real_, nrow = n, ncol = 3 * p)
    coln <- character(3 * p)
    for (j in seq_len(p)) {
        cols <- ((j - 1) * 3 + 1):((j - 1) * 3 + 3)
        out[, cols] <- cbind(add[, j], dom[, j], rec[, j])
        coln[cols] <- paste0(snp_names[j], c("", "_dom", "_rec"))
    }
    colnames(out) <- coln
...
coordinate_descent_horseshoe_perm_XI <- function(Y, X1, X2 = NULL, I_mat = NULL, Z = NULL,
                                                tau = 0.1, c = 1,
  ...
  # W = X2 covariates (kept as-is, not converted to dom/rec)
  if (!is.null(X2)) {
    Wmat <- as.matrix(X2)
  ...
  # SNP expanded matrix: 3 columns per SNP produced by make_dom_rec_matrix()
  X_snp <- make_dom_rec_matrix(X1)   # expected to return n x (3*p) with colnames
  ...
  # I_mat interactions / extra columns
  if (!is.null(I_mat)) {
  ...
  # Full design used for posterior calculations (SNP-expanded, W, I, Z)
  ...
  # small wrappers for log-likelihood / posterior evaluations
  log_lik <- function(Y, X_design, beta_full) {
    eta <- beta_full[1] + as.vector(X_design %*% beta_full[-1])
    ll_vec <- ifelse(eta >= 0,
                     Y * eta - (eta + log1p(exp(-eta))),
                     Y * eta - log1p(exp(eta)))
    sum(ll_vec)
  }

  # This relies on your existing function `logitic_our_log_posterior_reg_horseshoe`
  log_post_no_prior_on_Z <- function(Y, X_design, beta_full, tau, c) {
    # if there are no Z covariates, call original posterior directly
    if (q == 0) {
      return(logitic_our_log_posterior_reg_horseshoe(Y, X_design, beta_full, tau, c))
    }
    beta_zeroZ <- beta_full
    # z positions in beta_full: intercept (1) + 3*p SNPs + w Wcols + r Icols + 1 .. q
    z_start <- 1 + 3 * p + w + r + 1
    z_end <- 1 + 3 * p + w + r + q
    beta_zeroZ[z_start:z_end] <- 0

    lp_base <- logitic_our_log_posterior_reg_horseshoe(Y, X_design, beta_zeroZ, tau, c)

    ll_full <- log_lik(Y, X_design, beta_full)
    ll_zeroZ <- log_lik(Y, X_design, beta_zeroZ)
  ...
  # will store best per-SNP probs (rows = SNPs, cols = ADD,DOM,REC)
  ...
  # store best I permutation mapping when found
  ...
  while (perm_count < K) {
    ...
    # columns in expanded SNP matrix corresponding to the permuted SNP order:
    idx_perm_cols_snp <- unlist(lapply(permuted_snps,
                                      function(j) ((j - 1) * 3 + 1):((j - 1) * 3 + 3)))

    # --- 改为无条件按 permuted_snps 优先重排 I_mat（存在索引才取），剩下原序追加 ---
    if (r > 0) {
      # pref: permuted_snps 中那些在 I_mat 列范围内的索引（保持 permuted_snps 的顺序）
      pref <- intersect(permuted_snps, seq_len(r))
      # remaining: I_mat 中未被 pref 覆盖的列（保持原始顺序）
    ...
    # Build X_perm in the order: SNP-expanded (permuted), W (unpermuted), I_mat (permuted as above), Z
    if (r > 0 && q > 0) {
    ...
    for (iter in 1:max_iter) {
      ...
      obj_intercept <- function(b0) {
        tmp <- beta; tmp[1] <- b0
        -log_post_no_prior_on_Z(Y, X_perm, tmp, tau, c)
      }
      ...
      # SNP blocks (each block has 3 columns in X_perm)
      for (block_pos in seq_len(p)) {
        block_cols <- ((block_pos - 1) * 3 + 1):((block_pos - 1) * 3 + 3)
        beta_indices <- 1 + block_cols  # positions in beta vector (1 = intercept)
        ...
        for (mode in 1:3) {
          ...
          if (opt_mode$success) {
            ...
            lp_mode <- log_post_no_prior_on_Z(Y, X_perm, tmp_beta, tau, c)
      ...
      # Next update W (X2) coefficients (kept unpermuted)
      if (w > 0) {
      ...
      # Update I block (if present). I columns start after SNPs and W
      if (r > 0) {
      ...
      # Update Z covariates (if present). Z columns start after SNPs, W and I
      if (q > 0) {
    ...
    # SNPs: beta columns are arranged per permuted order in beta; map them to original SNP order
    for (orig in seq_len(p)) {
      pos <- which(permuted_snps == orig)  # position in permutation
      perm_cols <- ((pos - 1) * 3 + 1):((pos - 1) * 3 + 3)
      orig_cols <- ((orig - 1) * 3 + 1):((orig - 1) * 3 + 3)
      full_beta[1 + orig_cols] <- beta[1 + perm_cols]
    }

    # W (X2) - they are not permuted; place directly
    if (w > 0) {
      w_cols_perm_idx <- (3 * p + 1):(3 * p + w)
      full_beta[1 + w_cols_perm_idx] <- beta[1 + w_cols_perm_idx]
    }

    # I_mat: permuted mapping back to original I indices
    if (r > 0) {
      # I_idx_perm_cols maps positions in original I_mat to the permuted positions:
      # In X_perm I column at position perm_pos corresponds to original column I_idx_perm_cols[perm_pos].
      for (perm_pos in seq_len(r)) {
        orig_I_col <- I_idx_perm_cols[perm_pos]
        # full_beta I column index = 1 + 3*p + w + orig_I_col
        full_beta[1 + 3 * p + w + orig_I_col] <- beta[1 + 3 * p + w + perm_pos]
      }
    }

    # Z covariates (unpermuted)
    if (q > 0) {
    ...
    lp_now <- log_post_no_prior_on_Z(Y, X_new, full_beta, tau, c)
    ...
    if (lp_now > best_log_post) {
      ...
      # save the I_mat permutation order corresponding to this best solution
      if (r > 0) {
  ...
  # If I_mat was present, print the re-ordered I_mat column mapping for the best permutation
  if (r > 0) {
    if (!is.null(best_I_idx_perm_cols)) {
      # best_I_idx_perm_cols is the vector of original I_mat column indices in the order they were placed into X_perm (left-to-right)
      ...
      # Also show mapping: for each original I column, what is its position in X_perm (if useful)
  ...
  # Name coefficients in original order: intercept, SNP-expanded cols, W (X2), I_mat, Zmat
