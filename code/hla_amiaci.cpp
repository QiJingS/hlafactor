...
std::string normalize_locus_name(
    const std::string& locus)
{
    return strip_hla_prefix(locus);
}

std::string normalize_query_allele(
    const std::string& allele)
{
    std::string cleaned =
        strip_hla_prefix(allele);
...
std::string normalize_alignment_allele(
    const std::string& allele)
{
    return strip_hla_prefix(trim(allele));
}

std::string locus_from_alignment_path(
...
std::vector<
    std::pair<size_t, int>
> parse_protein_position_anchors(
    const std::string& prot_line)
{
    std::vector<
        std::pair<size_t, int>
    > anchors;
    ...
    >& anchors)
{
    ...
    for (const auto& [column, value]
        ...
        size_t relative_column =
            column - segment_start;

        auto it = std::lower_bound(
            residue_columns.begin(),
            residue_columns.end(),
            relative_column);

        if (it == residue_columns.end()
            || *it != relative_column) {
...
std::string decode_alignment_char(
    char reference_char,
    char allele_char)
{
    if (allele_char == '-') {
...
std::string make_genotype_token(
    char reference_char,
    char a1)
{
    return decode_alignment_char(
               reference_char,
               a1)
           + "/.";
}
    ...
    char a2)
{
    return decode_alignment_char(
               ...
               a1)
           + "/"
           + decode_alignment_char(
               reference_char,
               a2);
}

std::string resolve_allele_key(
    const AlignmentData& alignment,
    const std::string& raw_allele)
{
    std::string normalized =
        normalize_query_allele(
            raw_allele);
    ...
    for (const auto& entry
         : alignment.allele_to_aligned) {

        if (normalize_query_allele(
                entry.first)
            != normalized) {
        ...
        for (size_t i = 0;
             i < entry.second.size()
             && i < alignment
                         .has_reference_position
                         .size();
             ++i) {

            if (!alignment
                     .has_reference_position[i]) {
                continue;
            }
...
AlignmentData parse_imgt_alignment(
...
std::vector<ind_amiaci>
parse_mapping_reference(
    std::vector<samplerecord>& samples)
{
    ...
    for (const auto& sample : samples) {
        ...
        for (const auto& locus_pair
             : sample.loci) {

            std::string locus =
                normalize_locus_name(
                    locus_pair.first);

            auto alignment_it =
                alignments.find(locus);
    ...
    char* argv[])
{
    ...
    return parse_mapping_reference(
        samples);
}

void print_amiaci_preview(
    const std::vector<ind_amiaci>& aa_samples)
{
    for (const auto& ind
         : aa_samples) {
